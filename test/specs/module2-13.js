describe('Module 2-13: Lấy phiên bản hiệu lực ', () => {
    before(()=>{
        module1_6();
    })
    ...
    //Lấy phiên bản hiệu lực
    let fileRevisionActiveURL = process.env.URL_QLTLHHL;
    let btnGet = '[title="Lấy phiên bản hiệu lực"]';
    
  
    it('Lấy phiên bản hiệu lực thành công', () =>{

        browser.url(fileRevisionActiveURL);
        $(btnGet).click();

        //Expect output: chắc chắn trình duyệt điều hướng đến trang "Quản lý phiên bản hiệu lực" của quy trình đã chọn 
        expect(browser).toHaveUrl(process.env.URL_QLPBHLQT);
    })
