describe('Module 2-8: Xóa tài liệu ', () => {
    before(()=>{
        module1_5();
    })
    ...
    let btnDelete = '[title="Xóa chỉ số"]';
    let btnConfirm = '[class="btn btn-primary"]';
  
    it('Xóa tài liệu thành công', () =>{

        browser.url(fileRevisionActiveURL);
        $(btnEdit).click();

        $(btnDelete).waitForDisplayed(5000);
        $(btnDelete).click();
        browser.pause(2000);
       ...
       //Expect output: chắc chắn hiển thị thông báo xóa thành công 
